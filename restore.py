"""
Script for restoring database from Synology to a Docker container.
"""
...
def main():
    ...
    synology_server = filestation.FileStation(
        SYNOLOGY_HOST, SYNOLOGY_PORT, SYNOLOGY_USER, SYNOLOGY_PASSWORD, secure=True
    )
    ...
    file_info = synology_server.get_search_list(
        task_id=task_id, limit=1, sort_by="crtime", sort_direction="desc"
    )["data"]["files"][0]
    ...
    os.system(
        "cat {} | docker exec -i {} psql -U {}".format(
            file_info["name"], DOCKER_CONTAINER, POSTGRES_USER
        )
    )
